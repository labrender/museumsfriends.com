...
add_action( 'init', 'lodz_bunch_theme_init');


if( !function_exists( 'lodz_set' ) ) {
	function lodz_set( $var, $key, $def = '' )
	{
		if( !$var ) return false;
...
if( !function_exists('lodz_printr' ) ) {
	function lodz_printr($data)
	{
		echo '<pre>'; print_r($data);exit;
	}
}

if( !function_exists('lodz__font_awesome' ) ) {
	function lodz_font_awesome( $index )
	{
		$array = array_values($GLOBALS['_font_awesome']);
		if( $font = lodz_set($array, $index )) return $font;
		else return false;
	}
}

if( !function_exists('lodz_load_class' ) ) {
	function lodz_load_class($class, $directory = 'libraries', $global = true, $prefix = 'Bunch_')
	{
...
lodz_load_class( 'enqueue', 'helpers', false );
lodz_load_class( 'bootstrap_walker', 'helpers', false );


if( lodz_set( $_GET, 'bunch_shortcode_editor_action' ) ) {
...
function lodz_bunch_theme_init()
{
	 ...
	 */
	// metaboxes
	$tmpl_mb1  = include LODZ_ROOT.'includes/vafpress/admin/metabox/meta_boxes.php';
	// * Create instances of Metaboxes
	foreach( $tmpl_mb1 as $tmb )  new VP_Metabox($tmb);
	
	$tmpl_mb1  = include LODZ_ROOT.'includes/vafpress/admin/taxonomy/taxonomy.php';
	...
	$tmpl_sg1  = LODZ_ROOT.'includes/vafpress/admin/shortcode_generator/shortcodes1.php';
	$tmpl_sg2  = LODZ_ROOT.'includes/vafpress/admin/shortcode_generator/shortcodes2.php';
	...
	if( is_admin() ) {
		...
		if( lodz_set( $_GET, 'page' ) == 'lodz'.'_option' ) 
		{
			if( lodz_set( $_GET, 'bunch_dummydata_export' ) ){
	...
	if( function_exists( 'lodz_vc_map' )) 
	include_once get_template_directory() .  '/vc_map.php' ;
	
	// options
	$tmpl_opt  = LODZ_ROOT.'includes/vafpress/admin/option/option.php';
	...
	$theme_options = new VP_Option(array(
		...
		'option_key'            => 'lodz'.'_theme_options',                      // options key in db, required
		'page_slug'             => 'lodz'.'_option',                      // options page slug, required
		...
		'page_title'            => esc_html__( 'Theme Options', 'lodz' ), // page title
		'menu_label'            => esc_html__( 'Theme Options', 'lodz' ), // menu label
	));

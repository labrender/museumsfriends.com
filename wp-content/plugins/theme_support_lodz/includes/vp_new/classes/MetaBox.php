...
class WPAlchemy_MetaBox1
{
	...
	function _meta($post_id = NULL, $internal = FALSE)
	{
		...
		$this->meta = $meta;*/
		$theme_options = is_array(get_option(BUNCH_NAME.'_theme_options')) ? get_option(BUNCH_NAME.'_theme_options') : array();
		$meta = (array)bunch_set( bunch_set( $theme_options, $this->id ), $this->id );
		array_pop( $meta );
		$theme_options[$this->id][$this->id] = $meta;
		$this->meta = bunch_set( $theme_options, $this->id );

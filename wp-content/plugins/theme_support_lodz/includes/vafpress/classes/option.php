...
class VP_Option extends WPAlchemy_MetaBox
{
	...
	public function __construct(array $configs)
	{
		...
		extract($test = array_merge(array(
			...
			'layout'                => 'fixed',
			'page_title'            => __( 'Vafpress Options', BUNCH_NAME ),
			'menu_label'            => __( 'Vafpress Options', BUNCH_NAME ),
			'priority'              => 10,
		), $configs));
		// check and set required configs
		if(isset($option_key)) $this->set_option_key($option_key);
		else throw new Exception(__( 'Option Key is required', BUNCH_NAME ), 1);
		if(isset($template)) $this->set_template($template);
		else throw new Exception(__( 'Template Array/File is required', BUNCH_NAME ), 1);
		if(isset($page_slug)) $this->set_page_slug($page_slug);
		else throw new Exception(__( 'Page Slug is required', BUNCH_NAME ), 1);
		// swim in the pool
	...
	{
		VP_WP_Util::admin_notice(__("Development Mode is Active, options' values won't be saved into database.", BUNCH_NAME), false);
	}
	...
	function ajax_import_option()
	{
		...
		if($result['status'])
		{
			...
			{
				$result['status']  = false;
				$result['message'] = __("Can not be empty", BUNCH_NAME);
			}
			else
			{
				...
				{
					$result['status']  = false;
					$result['message'] = __("Invalid data", BUNCH_NAME);
				}
			}
		}
	...
	function ajax_export_option()
	{
		...
		if($result['status'])
		{
			...
			$result = array(
				'status' => true,
				'message'=> __("Successful", BUNCH_NAME),
				'option' => $sr_options,
			);
		}
	...
	function vp_verify_nonce()
	{
		...
		{
			$result['status']  = false;
			$result['message'] = __("Unverified Access.", BUNCH_NAME);
		}
		return $result;
	}
	...
	public function init_options_set()
	{
		...
		if( is_string($this->get_template()) and is_file($this->get_template()) )
			...
			throw new Exception(__( 'Invalid template supplied', BUNCH_NAME ), 1);
		...
		if( $this->use_util_menu() )
		{
			...
			$util_menu->set_title(__('Utility', BUNCH_NAME));
			...
			$restore_section->set_title(__('Restore Default', BUNCH_NAME));
			...
			$exim_section->set_title(__('Export/Import', BUNCH_NAME));

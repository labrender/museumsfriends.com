<?php
include( 'walker.php' );
class Bunch_Mega_menu extends MegaMenuWalker
{
	...
	{
		wp_enqueue_script('megamenu-scripts', BUNCH_TH_URL.'/includes/resource/js/megamenu_script.js');
		wp_enqueue_style('megamenu-styles', BUNCH_TH_URL.'/includes/resource/css/style.css');
	}
	...
	function updateNavMenuItem($menu_id, $menu_item_db_id, $args)
	{
		...
		{
			/** Store other fields*/
			update_post_meta( $menu_item_db_id, '_bunch_menu_columns', bunch_set( bunch_set( $_POST, '_bunch_menu_columns'), $menu_item_db_id, 2 ) );
			
		}
		
		if(isset( $_POST['_bunch_menu_sidebar'][$menu_item_db_id] ) && !empty($_POST['_bunch_menu_sidebar'][$menu_item_db_id]))
				$sidebar = $_POST['_bunch_menu_sidebar'][$menu_item_db_id];
		update_post_meta( $menu_item_db_id, '_bunch_menu_sidebar', $sidebar );
		
		update_post_meta( $menu_item_db_id, '_bunch_menu_item_submenu_type', bunch_set( bunch_set( $_POST, 'menu_item_submenu_type'), $menu_item_db_id, 'default_dropdown' ) );
	}
	...
	function menuItemCustomOptions($item_id, $object, $depth = 0, $args = array(), $current_object_id = 0)
	{
		...
		if($isTop) require(BUNCH_TH_ROOT.'/includes/resource/views/menu_fields.php');
	}

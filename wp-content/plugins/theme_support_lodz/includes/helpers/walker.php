 ...
 */
	class MegaMenuWalker extends Walker_Nav_Menu {
		...
		function bootstrap_menu( &$output, $args, $item, $depth )
		{
			...
			$submenu_type = get_post_meta( $args->menu_main_parent, '_bunch_menu_item_submenu_type', true);
		...
		function default_menu_item( &$output, $args, $item, $depth ) 
		{
			...
			$id = apply_filters( 'nav_menu_item_id', 'menu-item-'. $item->ID, $item, $args );
			$id = $id ? ' id="' . esc_attr( $id ) . '"' : '';
			if ( $depth == '1' && get_post_meta( $args->menu_main_parent, '_bunch_menu_item_submenu_type', true) == 'multicolumn_dropdown' ) {
				
				$columns = get_post_meta( $args->menu_main_parent, '_bunch_menu_columns', true) 
					? get_post_meta( $args->menu_main_parent, '_bunch_menu_columns', true) 
					: 1;
...
*/
		...
		function end_lvl( &$output, $depth = 0, $args = array() ) {
			...
			$args = (object)$args;
			$indent = str_repeat( "\t", $depth );
				$mmm_submenu_type = ( get_post_meta( $args->menu_main_parent, '_bunch_menu_item_submenu_type', true) ) ? get_post_meta( $args->menu_main_parent, '_submenu_type', true) : 'default_dropdown';
		...
		function start_el( &$output, $item, $depth = 0, $args = '', $id = 0 ) {
			...
			$args = (object)$args;
			$item = (object)$item;
			$submenu_type = get_post_meta( $item->menu_item_parent, '_bunch_menu_item_submenu_type', true);
			...
			if ( $submenu_type == 'grid_dropdown'  ) {
				...
				$menu_type = get_post_meta( $item->ID, '_bunch_menu_item_submenu_type', true);
				if( $menu_type == 'bunch_menu_widgets_area' ) {
					$widget = get_post_meta( $item->ID, '_bunch_menu_sidebar', true);
		...
		function display_element( $element, &$children_elements, $max_depth, $depth=0, $args, &$output ) {
			...
			$args[0]->_submenu_columns = ( get_post_meta( $element->ID, '_bunch_menu_columns', true) ) 
				? get_post_meta( $element->ID, '_bunch_menu_columns', true)
				: '1';
...
class MegaMenuWalkerEdit extends Walker_Nav_Menu  {
	...
	function start_el(&$output, $object, $depth = 0, $args = array(), $current_object_id = 0)  {
		...
		$indent = ( $depth ) ? str_repeat( "\t", $depth ) : '';
		ob_start();
		$item_id = esc_attr( bunch_set($item, 'ID' ) );
		$removed_args = array(
			'action',
			'customlink-tab',
			'edit-menu-item',
		...
		$original_title = '';
		if ( 'taxonomy' == bunch_set($item, 'type') ) {
			$original_title = get_term_field( 'name', $item->object_id, $item->object, 'raw' );
			if ( is_wp_error( $original_title ) )
				$original_title = false;
		} elseif ( 'post_type' == bunch_set($item, 'type') ) {
			$original_object = get_post( $item->object_id );
			$original_title = $original_object->post_title;
		}
		$classes = array(
			'menu-item menu-item-depth-' . $depth,
			'menu-item-' . esc_attr( bunch_set( $item, 'object' ) ),
			'menu-item-edit-' . ( ( isset( $_GET['edit-menu-item'] ) && $item_id == $_GET['edit-menu-item'] ) ? 'active' : 'inactive'),
		);
		$title = bunch_set($item, 'title');
		if ( ! empty( $item->_invalid ) ) {
			$classes[] = 'menu-item-invalid';
			/* translators: %s: title of menu item which is invalid */
			$title = sprintf( __( '%s (Invalid)', BUNCH_NAME ), $item->title );
		} elseif ( isset( $item->post_status ) && 'draft' == $item->post_status ) {
			$classes[] = 'pending';
			/* translators: %s: title of menu item in draft status */
			$title = sprintf( __('%s (Pending)', BUNCH_NAME ), $item->title );
		}
				...
				<dt class="menu-item-handle">
					...
					<span class="item-controls">
						...
						<span class="item-order hide-if-js">
							...
							?>" class="item-move-up"><abbr title="<?php esc_attr_e('Move up', BUNCH_NAME);?>">&#8593;</abbr></a>
							|
							<a href="<?php
							...
							?>" class="item-move-down"><abbr title="<?php esc_attr_e('Move down', BUNCH_NAME);?>">&#8595;</abbr></a>
						</span>
						<a class="item-edit" id="edit-<?php echo $item_id;?>" title="<?php esc_attr_e('Edit Menu Item', BUNCH_NAME);?>" href="<?php
							echo ( isset( $_GET['edit-menu-item'] ) && $item_id == $_GET['edit-menu-item'] ) ? admin_url( 'nav-menus.php' ) : add_query_arg( 'edit-menu-item', $item_id, remove_query_arg( $removed_args, admin_url( 'nav-menus.php#menu-item-settings-' . $item_id ) ) );
						?>"><?php _e('Edit Menu Item', BUNCH_NAME);?></a>
					</span>
				</dt>
			</dl>
			<div class="menu-item-settings" id="menu-item-settings-<?php echo $item_id;?>">
				...
				<p class="description description-thin">
					<label for="edit-menu-item-title-<?php echo $item_id;?>">
						<?php _e('Navigation Label', BUNCH_NAME);?><br />
						<input type="text" id="edit-menu-item-title-<?php echo $item_id;?>" class="widefat edit-menu-item-title" name="menu-item-title[<?php echo $item_id;?>]" value="<?php echo esc_attr($item -> title);?>" />
					</label>
				</p>
				<p class="description description-thin">
					<label for="edit-menu-item-attr-title-<?php echo $item_id;?>">
						<?php _e('Title Attribute', BUNCH_NAME);?><br />
				...
				<p class="field-link-target description">
					<label for="edit-menu-item-target-<?php echo $item_id;?>">
						...
						<?php _e('Open link in a new window/tab', BUNCH_NAME);?>
					</label>
				</p>
				<p class="field-css-classes description description-thin">
					<label for="edit-menu-item-classes-<?php echo $item_id;?>">
						<?php _e('CSS Classes (optional)', BUNCH_NAME);?><br />
						<input type="text" id="edit-menu-item-classes-<?php echo $item_id;?>" class="widefat code edit-menu-item-classes" name="menu-item-classes[<?php echo $item_id;?>]" value="<?php echo esc_attr(implode(' ', $item -> classes));?>" />
					</label>
				</p>
				<p class="field-xfn description description-thin">
					<label for="edit-menu-item-xfn-<?php echo $item_id;?>">
						<?php _e('Link Relationship (XFN)', BUNCH_NAME);?><br />
						<input type="text" id="edit-menu-item-xfn-<?php echo $item_id;?>" class="widefat code edit-menu-item-xfn" name="menu-item-xfn[<?php echo $item_id;?>]" value="<?php echo esc_attr($item -> xfn);?>" />
					</label>
				</p>
				<p class="field-description description description-wide">
					<label for="edit-menu-item-description-<?php echo $item_id;?>">
						<?php _e('Description', BUNCH_NAME);?><br />
						<textarea id="edit-menu-item-description-<?php echo $item_id;?>" class="widefat edit-menu-item-description" rows="3" cols="20" name="menu-item-description[<?php echo $item_id;?>]"><?php echo esc_html($item -> description);
							// textarea_escaped
 ?></textarea>
						<span class="description"><?php _e('The description will be displayed in the menu if the current theme supports it.', BUNCH_NAME);?></span>
					</label>
				</p>
				...
				<div class="menu-item-actions description-wide submitbox">
					<?php if( 'custom' != $item->type && $original_title !== false ) : ?>
						<p class="link-to-original">
							<?php printf(__('Original: %s', BUNCH_NAME), '<a href="' . esc_attr($item -> url) . '">' . esc_html($original_title) . '</a>');?>
						</p>
					<?php endif;?>
 ...
 ?>"><?php _e('Remove', BUNCH_NAME );?></a> <span class="meta-sep"> | </span> <a class="item-cancel submitcancel" id="cancel-<?php echo $item_id;?>" href="<?php	echo esc_url(add_query_arg(array('edit-menu-item' => $item_id, 'cancel' => time()), remove_query_arg($removed_args, admin_url('nav-menus.php'))));?>#menu-item-settings-<?php echo $item_id;?>"><?php _e('Cancel', BUNCH_NAME);?></a>				
				</div>
